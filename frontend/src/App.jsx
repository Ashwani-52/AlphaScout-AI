...
export default function App() {
  ...
  // Programmatic Google Sign-In Hook
  const login = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        setError('Authentication failed. Could not retrieve Google profile.');
      }
    },
    onError: (error) => {
  ...
  const handleInputInteraction = (e) => {
    if (!user) {
      ...
      login(); // Instantly trigger Google Login window
    }
  };
      ...
      <header className="w-full max-w-[1560px] mx-auto px-6 py-6 flex justify-between items-center z-10">
        ...
        <div className="flex items-center gap-4">
          ...
          {user ? (
            ...
            <button 
              onClick={() => login()} 
              className={`flex items-center gap-2 bg-white hover:bg-slate-50 border border-slate-200 shadow-sm text-slate-700 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer ${shakeLoginButton ? 'animate-shake' : ''}`}
            >
